...
impl CPU6502{
    ...
    pub fn GetFlag(&mut self, flag: Flags) -> u8{
    ...
    pub fn SetFlag(&mut self, flag: Flags, v: bool){
        if v == true 
        {
    ...
    pub fn disassemble(&mut self, nStart: u16, nStop: u16) -> HashMap<u32, String>{
        ...
        while addr <= nStop as u32{
            ...
            if self.lookup[opcode as usize].addr_name == "IMP"{
                ...
                addr = addr + 1;
                d.push_str("$");		
                let hexVal = ((hi as u16) << 8) | lo as u16;
                format = format!("{:X}", hexVal);			
                d.push_str(&format);
                d.push_str(" {ABS}");
...
impl CPU6502{
    ...
    fn ADC(&mut self) -> u8{
        self.fetch();
        let flagC = self.GetFlag(Flags::C);
        let tmp = (self.a + self.fetched + flagC) as u16;

        if tmp > 255 {
            self.SetFlag(Flags::C, true);
        }else{
            self.SetFlag(Flags::C, false);
        }
    ...
    //Clear decimal flag
    fn CLD(&mut self) -> u8{
        self.SetFlag(Flags::D, false);
        return 0;
    }

    //Clear interrupt flag
    fn CLI(&mut self) -> u8{
        self.SetFlag(Flags::I, false);
        return 0;
    }
